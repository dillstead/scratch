...
typedef struct my_sem_t Semaphore;

Semaphore *make_semaphore (int n)
{
    Semaphore *sem = check_malloc (sizeof(Semaphore));
    int ret = my_sem_init(sem, n);
    if (ret == -1) perror_exit ("sem_init failed");
    return sem;
}

int my_sem_signal(Semaphore *sem)
{
    return my_sem_post(sem);
}
...
void child_code (Shared *shared)
{
    ...
    while (1) {
        my_sem_wait(shared->mutex);
        if (shared->counter >= shared->end) {
            my_sem_signal(shared->mutex);
            return;
        }

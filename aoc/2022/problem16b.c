// gcc -Werror -pedantic -std=c99 -Wall -Wextra -fsanitize=address,undefined -g -o problem16b problem16b.c
#include <stdio.h>
#include <stdlib.h>
#include <stdbool.h>
#include <string.h>
...
static int calculate_max_flow(struct valve *valves, int *solution, int pos, int flow,
                              int time, int *flow_valves, int num_flow_valves,
                              int min_dists[][NUM_VALVES], int max_flow)
{
    ...
    if (num_nexts > 0)
    {
        for (int i = 0; i < num_nexts; i++)
        {
            ...
            new_flow = calculate_max_flow(valves, solution, pos + 1, new_flow, next_valves[i].new_time,
                                          flow_valves, num_flow_valves, min_dists, max_flow);
...
static void generate_subsets(int *solution, int solution_pos, int pos, int num_flow_valves,
                             int *flow_valves, int *cur_subset, struct flow_valve_subset *flow_valve_subsets)
{
    if (pos == num_flow_valves)
    {
        flow_valve_subsets[*cur_subset].num_flow_valves = solution_pos;
        memcpy(flow_valve_subsets[*cur_subset].flow_valves, solution,
               solution_pos * sizeof *solution);
        (*cur_subset)++;
    }
    else
    {
        solution[solution_pos] = flow_valves[pos];        
        generate_subsets(solution, solution_pos + 1, pos + 1, num_flow_valves,
                         flow_valves, cur_subset, flow_valve_subsets);
        generate_subsets(solution, solution_pos, pos + 1, num_flow_valves,
                         flow_valves, cur_subset, flow_valve_subsets);
    }
}
...
int main(void)
{
    ...
    struct flow_valve_subset *flow_valve_subsets;
    int cur_subset = 0;
    int num_subsets;
    int max_flow;
    int flow;
    ...
    num_subsets = 1 << num_flow_valves;
    flow_valve_subsets = calloc(num_subsets, sizeof *flow_valve_subsets);
    generate_subsets(solution, 0, 0, num_flow_valves, flow_valves, &cur_subset, flow_valve_subsets);
    solution[0] = 0;
    for (int i = 0; i < num_subsets / 2; i++)
    {
        flow = calculate_max_flow(valves, solution, 1, 0, NUM_TIME, flow_valve_subsets[i].flow_valves,
                                  flow_valve_subsets[i].num_flow_valves, min_dists, 0)
            + calculate_max_flow(valves, solution, 1, 0, NUM_TIME, flow_valve_subsets[num_subsets - 1 - i].flow_valves,
                                 flow_valve_subsets[num_subsets - 1 - i].num_flow_valves, min_dists, 0);
        if (flow > max_flow)
        {
            max_flow = flow;
        }
